...
class SuperSprite(pl.sprite.Sprite):
    ...
    def __init__(self, x = 0, y = 0, image = None, frames=1,  speed: int = 0, \
        ...
        # slice the frames
        self.images = self.createFrames(frames, image)
...
#----- IMAGE FUNCTIONS -----#
    ...
    def createFrames(self, frames: int, img):
        imageList = pl.resource.image(img)
        frame_list = pl.image.ImageGrid(imageList, rows=1, columns=frames)
        return frame_list
